...
def measure_layer_for_pruning(layer, x):
    """Measure FLOPs and params for a layer - FIXED to handle layers without bias"""
    ...
    # ops_conv (Conv1d for ECG)
    if type_name in ['Conv1d']:
        out_length = int((x.size()[2] + 2 * layer.padding[0] - layer.kernel_size[0]) /
                         layer.stride[0] + 1)
        layer.flops = layer.in_channels * layer.out_channels * layer.kernel_size[0] * \
                      out_length / layer.groups * multi_add
        layer.params = get_layer_param(layer)

    # ops_conv (Conv2d for compatibility)
    ...
    # ops_linear - FIXED: Handle bias=False
    elif type_name in ['Linear']:
        weight_ops = layer.weight.numel() * multi_add
        # Check if bias exists before accessing it
        bias_ops = layer.bias.numel() if layer.bias is not None else 0
        layer.flops = weight_ops + bias_ops
        layer.params = get_layer_param(layer)

    # ops_nothing
    elif type_name in ['BatchNorm1d', 'BatchNorm2d', 'Dropout', 'Dropout2d', 'DropChannel']:
        layer.params = get_layer_param(layer)
        layer.flops = 0

    # unknown layer type - set to 0
    else:
        layer.params = get_layer_param(layer)
        layer.flops = 0
